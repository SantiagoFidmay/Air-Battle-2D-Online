class Obj {
    ...
    des_img() {
        ...
        if(this instanceof AviaoInimigo || this instanceof Boss) des.rotate(Math.PI);
...
class Aviao extends Obj {
    constructor(x, y, w, h, img, playerColor = null) {
        super(x, y, w, h, img);
        this.dir = 0; this.vida = 5; this.pontos = 0; this.fase = 1;
        this.ultimoTiro = 0; this.cooldown = 250; this.velAtual = 8;
        this.tiroDuplo = false; this.escudo = false;
        this.playerColor = playerColor; // 'red' P1 | 'blue' P2
    }
    ...
    des_img() {
        ...
        // Tag minimalista P1/P2 acima do avião
        if(this.playerColor) {
            let label = this.playerColor === 'red' ? 'P1' : 'P2';
            let cor   = this.playerColor === 'red' ? '#ff4444' : '#4c9fff';
            ...
            des.fillText(label, cx + 1, ty + 1); // sombra
            des.fillStyle = cor;
            des.fillText(label, cx, ty);
            des.restore();
        }
    }
...
class AviaoInimigo extends Obj {
    constructor(x, y, w, h, img, id = null) {
        super(x, y, w, h, img);
        this.vel = 3;
...
class Tiro {
    ...
    des_quad() {
        if(this.owner && this.owner.playerColor === 'red')       des.fillStyle = "#ff6666";
        else if(this.owner && this.owner.playerColor === 'blue') des.fillStyle = "#66aaff";
        else                                                      des.fillStyle = "yellow";
